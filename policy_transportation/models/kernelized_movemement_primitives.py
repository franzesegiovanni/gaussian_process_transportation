...
class KMP():
    ...
    def fit(self, source_distribution, target_distribution, training_traj, kernel=C(0.1, constant_value_bounds=[0.1,  10]) * RBF(length_scale=[0.1], length_scale_bounds=[0.3, 1]) + WhiteKernel(0.00001)):
        ...
        # if self.periodic:
        #     #covert time to be periodic
        #     time=[]
        #     time.append(np.cos(2*np.pi*self.time))
        #     time.append(np.sin(2*np.pi*self.time))
        #     self.time=np.vstack(time).T
        #     self.time=self.time.reshape(-1,2)
        ...
        k_star= self.gp.kernel(self.time, self.time[self.mask_traj,:])
        K_star_star = self.gp.kernel(self.time[self.mask_traj,:], self.time[self.mask_traj,:])
    ...
    def predict(self, X, return_std=False):
        ...
        if return_std:
            self.mean, std= self.gp.predict(self.time, return_std=True)
            ...
            self.mean, _ = self.gp.predict(self.time, return_std=True)
            return self.mean
