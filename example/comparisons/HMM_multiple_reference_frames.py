...
# results_df=  np.empty( ( len(demos_x) ,0 ))
# results_area=np.empty(( len(demos_x) , 0))
# results_dtw=np.empty(( len(demos_x) , 0))
# results_fde=np.empty((  len(demos_x) , 0))

length = len(demos_xdx_augm)-1

# Create an empty one-dimensional list with length using list comprehension
results_df = [[] for _ in range(length)]
results_area= [[] for _ in range(length)]
results_dtw= [[] for _ in range(length)]
results_fde= [[] for _ in range(length)]
results_fad= [[] for _ in range(length)]
