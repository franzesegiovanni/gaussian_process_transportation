...
angle_data = lasa.DataSet.BendedLine
...
grad = gp.derivative_of_variance(X_pred).T
vect=np.sqrt(grad[:,0]**2+grad[:,1]**2+grad[:,2]**2)
dX=mean[:,0]-2*std[:,0]*grad[:,0]/vect
dY=mean[:,1]-2*std[:,1]*grad[:,0]/vect
...
mean=gp_deltaX1.predict(X1_pred)
grad=gp.derivative_of_variance(X1_pred).T
vect=np.sqrt(grad[:,0]**2+grad[:,1]**2+grad[:,2]**2)
dX=mean[:,0]-2*std[:,0]*grad[:,0]/vect
dY=mean[:,1]-2*std[:,1]*grad[:,1]/vect
dZ=mean[:,2]#-2*std*grad[:,2,0]/vect
# fig = plt.figure()
...
# ax.set_zlim(-0.2, 0.2)
plt.savefig(current_file_directory+'/figs/LASA_deformed.pdf', bbox_inches='tight', pad_inches=0, dpi=600)
plt.show()