...
k_deltaX = C(constant_value=np.sqrt(0.1))  * Matern(1*np.ones(2), length_scale_bounds=[10,500], nu=2.5) + WhiteKernel(0.01) 
...
k_deltaX1 = C(constant_value=np.sqrt(0.1))  * Matern(1*np.ones(2), length_scale_bounds=[10,500], nu=2.5) + WhiteKernel(0.01 )    
...
#save figure
fig.savefig(source_path+'/pictures/transportation_scheme_svgp.pdf',bbox_inches='tight')
...
# rest of the code
fig.savefig(source_path+'/pictures/uncertainty_surface_svgp.pdf',bbox_inches='tight', pad_inches=0.3)
plt.show()
