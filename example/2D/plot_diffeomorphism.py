...
cmap='PiYG'
axs[0].scatter(grid[:,0],grid[:,1], c=colors, cmap=cmap)
axs[0].legend()
for i in range(num_points):
        axs[0].plot(X[i,:],Y[i,:], color=[0,0,0])
        axs[0].plot(X[:,i],Y[:,i], color=[0,0,0])
axs[0].scatter(source_distribution[:,0],source_distribution[:,1], color=[1,0,0], label='Source Distribution')
axs[0].set_xlim(x_lim)
axs[0].set_ylim(y_lim)
axs[0].set_title('Source distribution', fontsize=20)
axs[0].legend()
# plt.axis('equal')
...
axs[1].scatter(grid_new[:,0],grid_new[:,1],c=colors, cmap=cmap)
...
axs[1].scatter(target_distribution[:,0],target_distribution[:,1], color=[0,0,1], label='Target Distribution')
axs[1].set_title('Affine Transformation', fontsize=20)
axs[1].set_yticks([])
...
axs[2].set_title(' GP Transportation', fontsize=20)
axs[2].legend()
axs[2].set_yticks([])
