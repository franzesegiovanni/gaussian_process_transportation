...
#%% Fit a GP to new surface, find delta pointcloud between old and sampled new surface
k_S1 = C(constant_value=np.sqrt(0.1))  * Matern(1*np.ones(1), nu=1.5) + WhiteKernel(0.01 ) #this kernel works much better!    
gp_S1=GPR(kernel=k_S1)
gp_S1.fit(S1[:,0].reshape(-1,1),S1[:,1].reshape(-1,1))

S1_sampled = np.zeros((len(S[:,0]),2))
deltaPC = np.zeros((len(S[:,0]),2))

for i in range(len(S[:,0])):
    pos=np.array(S[i,0]).reshape(1,-1)
    [y_S1, std_S1]=gp_S1.predict(pos)
    S1_sampled[i,:]= (S[i,0],y_S1)
    
deltaPC = S1_sampled - S
...
gp_deltaX1.plot_vector_field_minvar(x_grid,y_grid,X1,S1)
